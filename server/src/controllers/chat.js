...
const RAIN_STATE = {
  ...
  verify_req: false, // If the user has to be verified or not to join the rain
};
...
const listen = io => {
  ...
  const endCurrentRain = async () => {
    ...
    try {
      ...
      for (let index = 0; index < RAIN_STATE.players.length; index++) {
        ...
        // Update document
        await User.updateOne({ _id: player }, { 
          $inc: 
          { 
            wallet: profit, 
            wagerNeededForWithdraw: user.wagerNeededForWithdraw < 0 ? Math.abs(user.wagerNeededForWithdraw) + (profit * 1) : (profit * 1), 
          } 
        });
  ...
  // Start a new rain
  const startNewRain = (timeInMs, prize, verify_req) => {
    ...
    RAIN_STATE.verify_req = verify_req;
  ...
  const endActiveTrivia = async gameId => {
    try {
      ...
      for (let index = 0; index < activeTrivia.winners.length; index++) {
        ...
        const user = await User.findOne({ _id: winnerId });
        // Update document
        await User.updateOne(
          { _id: winnerId },
          { $inc: { wallet: activeTrivia.prize, wagerNeededForWithdraw: user.wagerNeededForWithdraw < 0 ? Math.abs(user.wagerNeededForWithdraw) + (activeTrivia.prize * 1) : (activeTrivia.prize * 1), } }
        );
        ...
        const data = {
          // content: `<@&${config.discord.trivia_id}>`,
          embeds: [
            {
              title: 'New trivia winner 🕵️',
              // color: parseInt("5DADEC", 16),
              fields: [
                {
                  name: 'question: ',
                  value: `\`${activeTrivia.question}\``,
                  inline: true,
                },
                {
                  name: 'answer: ',
                  value: `\`${activeTrivia.answer}\``,
                  inline: true,
                },
                {
                  name: 'value: ',
                  value: `\`${activeTrivia.prize} cash\``,
                  inline: true,
                },
                {
                  name: 'winner: ',
        ...
        try {
          await axios.post(config.discord.notificationWebhook, data);
        } catch (error) {
          console.log("Error notifing discord: " + error);
        }
  ...
  io.of("/chat").on("connection", socket => {
    ...
    socket.on("send-chat-message", async content => {
      ...
      try {
        ...
        function isBoolean(arg) {
          ...
          if (lowercasedArg === 'true' || lowercasedArg === 'false') {
            return true; // The argument is a boolean
          } else {
            return false; // The argument is not a boolean
          }
        }
        ...
        if (dbUser.rank === 5 && command.includes(".roll")) {
          // Validate admin input
          if (!args[1])
            return socket.emit(
              "notify-error",
              "Please specify <cupsgameid>"
            );
          
          const gameid = args[1];
          const game = await CupsGame.findOne({ _id: gameid });

          io.of("/cups").emit("game-rolling", gameid);
          console.log(colors.cyan("Cups >> Rolling game"), gameid);
          ...
          const winner = players.find(player => player.color === winningCup);
          ...
          return socket.emit("notify-success", "Successfully rolled game!");
        }
        ...
        if (dbUser.rank === 5 && command.includes(".create-rain")) {
          ...
          if (!args[1] || !args[2] || !args[3])
            return socket.emit(
              "notify-error",
              "Please specify <timeInSeconds>, <prize>, and <req_verify>"
            );
          ...
          if (!isBoolean(args[3]))
            return socket.emit(
              "notify-error",
              "<req_verify> must be \"true\" or \"false\"!"
            );  
          ...
          // Create a new game
          startNewRain(parseInt(args[1]) * 1000, parseFloat(args[2]), verify);

          const data = {
            content: `<@&${config.discord.rain_id}>`,
            embeds: [
              {
                title: 'New rain 🌧️',
                color: parseInt("5DADEC", 16),
                fields: [
                  {
                    name: 'ends: ',
                    value: `\`${(parseInt(args[1]))/60} min\``,
                    inline: true,
                  },
                  {
                    name: 'value: ',
                    value: `\`${parseFloat(args[2])} cash\``,
                    inline: true,
                  },
                  {
                    name: 'verfiy_req: ',
    ...
    socket.on("enter-rain", async recaptchaResponse => {
      // Validate user input
      if(!user.hasVerifiedAccount && RAIN_STATE.verify_req) {
        return socket.emit(
          "rain-join-error",
          "Your account must be verified to join this rain!"
        );
      }
