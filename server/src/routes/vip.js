...
router.post("/claim", validateJWT, async (req, res, next) => {
  try {
    ...
    await User.updateOne(
      ...
      {
        $inc: { 
          wallet: Math.abs(user.rakebackBalance),
          wagerNeededForWithdraw: user.wagerNeededForWithdraw < 0 ? Math.abs(user.wagerNeededForWithdraw) + (user.rakebackBalance * 1) : (user.rakebackBalance * 1),
        },
