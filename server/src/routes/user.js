...
const SlotsWin = require("../models/SlotsWin");
...
router.get("/history", middleware, async (req, res, next) => {
  try {
    ...
    // Get user crash games
    // const crashGames = await CrashGame.find({ "players._id": user.id, status: 4 }).lean();
    // const crashGamesWon = await CrashGame.find({ ...query, status: 4 }).lean();

    // Get user slots games 
    const slotsGames = await SlotsGame.find({ user: user.id }).lean();
    const slotsGamesWon = await SlotsWin.find({ user: user.id }).lean();
    ...
    const gamesPlayed = [
      ...slotsGames.map(g => ({
        ...g,
        gamemode: "slots",
        ownBetAmount: (g.betAmount*1.4).toFixed(2),
      })),
      //...crashGames.map(g => ({
      //  ...g,
      //  gamemode: "crash",
      ...
      if (!user._affiliatedBy) {
        ...
        await User.updateOne(
          { _id: user.id },
          {
            $inc: { 
              wallet: 0.5,
              wagerNeededForWithdraw: user.wagerNeededForWithdraw < 0 ? Math.abs(user.wagerNeededForWithdraw) + 0.5 : 0.5,
            },
            $set: {
    ...
    await User.updateOne(
      { _id: user.id },
      {
        $inc: {
          ...
          wagerNeededForWithdraw: user.wagerNeededForWithdraw < 0 ? Math.abs(user.wagerNeededForWithdraw) + (affiliateRevenue * 1) : (affiliateRevenue * 1),
        },
      }
    );
    insertNewWalletTransaction(
